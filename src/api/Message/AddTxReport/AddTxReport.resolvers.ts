...
import { AddTxReportMutationArgs, AddTxReportResponse } from "../../../types/graph";
...
const resolvers: Resolvers = {
  Mutation: {
    AddTxReport: privateResolver(
      async (
        parent,
        args: AddTxReportMutationArgs,
        { req, res }
      ): Promise<AddTxReportResponse> => {
        ...
        try {
          ...
          let promises: Promise<AddTxReportResponse>[]= []

          msgSend.init();

          let eventOnPromise = new Promise<AddTxReportResponse>((resolve,reject)=>{
          ...
          //delay_return(timeOut);
           ...
           promises.push(new Promise<AddTxReportResponse>((resolve, reject)=>{
              setTimeout(()=>{
                resolve({
                  ok: false,
                  error: "timeout"
